...
class baseMAGISolver():
    ...
    def _configure_polymorphism(self, to_tensor, to_arr, is_tensor, vmap, replicate, pad_tensor, normal, concat, reshape,
            ...
            prepare_particles, tensor_abs, tensor_allsmall, tensor_max, clone, gradient_step, Adam, autograd, stack, update_tensor, clip, slice_2,
                batch_mahalanobis, compute_cov_prior, inverse, compute_subspace):
        '''
        *** HELPER METHOD: NOT USER SAFE. DO NOT CALL. ***
        ...
        ### pSVGD modifications
        self.batch_mahalanobis = batch_mahalanobis
        self.compute_cov_prior = compute_cov_prior
        self.inverse = inverse
        self.compute_subspace = compute_subspace
    ...
    def initialize_particles(self, k_0, dtype, device=None, init_sd=0.2, random_seed=None, mitosis=False):
        ...
        ### pSVGD modifications
        self.cov_prior = self.compute_cov_prior(self.p, self.unknown_sigmas, self.I, self.phis,
                                                v=2.01, dtype=self.dtype, device=self.device)
        self.cov_prior_inv = self.inverse(self.cov_prior)
        ...
        *** HELPER METHOD: USER SAFE. ***
        ...
        if self.unknown_sigmas is not None:
            self.update_tensor(particles, np.arange(self.p+self.D*self.n, particles.shape[1]),
                    particles[:,self.p+self.D*self.n:] * self.to_tensor(particles[:,self.p+self.D*self.n:] > 0, dtype=self.dtype))
    ...
    def svgd_kernel(self, weights, Lam_I=None, h=-1):
        '''
        *** HELPER METHOD: USER SAFE BUT UNLIKELY TO BE USED. ***
        ...
        '''
        ### pSVGD modifications
        L2sq = self.batch_mahalanobis(weights, Lam_I)
        ...
        dxkxy += weights * self.tile(sumkxy, [1, weights.shape[1]])
        dxkxy *= 2/h
    ...
    def solve(self, optimizer, optimizer_kwargs=dict(), max_iter=10, subspace_updates=1000, alpha=0.01,
        ...
        subspace_updates (int) : number of times to update projection subspace
        ...
        ''' 
        ### pSVGD modifications
        for i in range(subspace_updates+1):
            # compute projection subspace
            grad_particles = self.gradient(self.particles)
            lam, psi, Lam_I, ss_dim = self.compute_subspace(grad_particles, self.cov_prior_inv, alpha)

            # get projection weights and orthogonal components
            weights = self.particles @ psi
            x_orth = self.particles - weights @ psi.T

            if optimizer_kwargs.pop('params', None):
                opt = optimizer(params=[weights], **optimizer_kwargs)
                optimizer_kwargs['params'] = True
            else:
                opt = optimizer(**optimizer_kwargs)

            for iteration in range(max_iter):
                grad_weights = -self.gradient(weights @ psi.T + x_orth) @ psi
                if not self.MAP:
                    kxy, dxkxy = self.svgd_kernel(weights, Lam_I, h=bandwidth)
                    grad_weights = (kxy @ grad_weights - dxkxy) / self.k
                    
                self.gradient_step(opt, grad_weights, weights)

            # reconstruct particles
            self.particles = weights @ psi.T + x_orth

            # check for convergence
            grad_particles = -self.gradient(self.particles)
            if not self.MAP:
                kxy, dxkxy = self.svgd_kernel(self.particles, None, h=bandwidth)
                grad_particles = (kxy @ grad_particles - dxkxy) / self.k

            m = self.tensor_max(self.tensor_abs(grad_particles))
            if monitor_convergence:
                print(f'Projection {i} finished with max grad = {m:.5f}, subspace dimension = {ss_dim} / {self.particles.shape[1]}')
        ...
        return Xs, thetas, sigmas, ss_dim
